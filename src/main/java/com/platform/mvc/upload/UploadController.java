...
public class UploadController extends BaseController {
	...
	public UploadController() {
		if (path_web_root == null || path_web_root.isEmpty()) {
			...
			web_root.append(PathKit.getWebRootPath()).append(File.separator).append("files").append(File.separator)
					.append("upload");
			path_web_root = web_root.toString();

			log.info("保存到/WebRoot/WEB-INF/files/upload");
			StringBuilder web_inf = new StringBuilder();
			web_inf.append(PathKit.getWebRootPath()).append(File.separator).append("WEB-INF").append(File.separator)
	...
	public void index() {
		String path = null;
		if (null != pathType && pathType.equals("webRoot")) {// pathType =
																// "webRoot";
			path = path_web_root;

		} else if (null != pathType && pathType.equals("webInf")) {// pathType =
																	// "webInf";
			path = path_web_inf;
		...
		List<UploadFile> files = getFiles(path, PropKit.getInt(ConstantInit.config_maxPostSize_key),
				ToolString.encoding);
	 ...
	 */
	public void delete() {
		UploadService.service.delete("pt_upload", "ids", getPara() == null ? ids : getPara());
