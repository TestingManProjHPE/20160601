 ...
 *         描述： 表单
 * 
 *         <input type="hidden" name="groupList[0].ids" value="111"/> <input
 *         type="text" name="groupList[0].names" value="222"/>
 * 
 *         <input type="hidden" name="groupList[1].ids" value="333"/> <input
 *         type="text" name="groupList[1].names" value="444"/>
 * 
 *         <input type="hidden" name="groupList[3].ids" value="555"/> <input
 *         type="text" name="groupList[3].names" value="666"/>
 * 
 *         调用方法
 * 
 *         List<Group> groupList = ToolModelInjector.injectModels(getRequest(),
 *         Group.class, "groupList");
 * 
 *         List<Group> groupList = ToolModelInjector.injectModels(getRequest(),
 *         Group.class); // 默认的prefix是Model类的首字母小写加上List
 * 
 */
public class ToolModelInjector {
	 ...
	 * 
	 */
	public static <T extends BaseModel<T>> List<T> injectModels(final HttpServletRequest request,
	 ...
	 */
	public static <T extends BaseModel<T>> List<T> injectModels(final HttpServletRequest request,
			Class<? extends T> modelClass, String prefix) {
